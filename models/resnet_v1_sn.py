...
model_name = {
     'resnetv1sn50': 'resnet-50-sn-8-2-75.13.pth'
}
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, using_moving_average=True, using_bn=True,  last_gamma=True, syncbn=True):
        super(BasicBlock, self).__init__()
        self.conv1 = conv3x3(inplanes, planes, stride)
        self.sn1 = sn.SwitchNorm2d(planes, using_bn=using_bn)
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = conv3x3(planes, planes)
        self.sn2 = sn.SwitchNorm2d(planes, using_bn=using_bn, last_gamma=last_gamma)
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, using_moving_average=True, using_bn=True, last_gamma=True):
        ...
        self.conv2 = nn.Conv2d(planes, planes, kernel_size=3, stride=stride,
                               padding=1, bias=False)
        self.sn2 = sn.SwitchNorm2d(planes, using_bn=using_bn)
        self.conv3 = nn.Conv2d(planes, planes * 4, kernel_size=1, bias=False)
        self.sn3 = sn.SwitchNorm2d(planes * 4, using_bn=using_bn, last_gamma=last_gamma)
        self.relu = nn.ReLU(inplace=True)
        self.downsample = downsample
        self.stride = stride
...
class ResNetV1SN(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, using_moving_average=True, using_bn=True, last_gamma=True):
        ...
        self.conv1 = nn.Conv2d(3, 64, kernel_size=7, stride=2, padding=3,
                               bias=False)
        self.sn1 = sn.SwitchNorm2d(64, using_bn=self.using_bn)
        self.relu = nn.ReLU(inplace=True)
    ...
    def _make_layer(self, block, planes, blocks, stride=1):
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            downsample = nn.Sequential(
                ...
                sn.SwitchNorm2d(planes * block.expansion, using_bn=self.using_bn),
            )

        layers = []
        layers.append(block(self.inplanes, planes, stride, downsample, using_moving_average=self.using_moving_average, using_bn=self.using_bn, last_gamma=self.last_gamma))
...
def resnetv1sn50(pretrained=True, **kwargs):
    """Constructs a ResNetV1SN-50 model using switchable normalization.
    """
    model = ResNetV1SN(Bottleneck, [3, 4, 6, 3], **kwargs)
    if pretrained:
        model.load_state_dict(load_weight(model_name['resnetv1sn50']), strict=True)        
    return model
...
def load_weight(model_name, model_dir='pretrained_sn', map_location=None):
