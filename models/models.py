...
def conv3x3_sn_relu(in_planes, out_planes, stride=1):
    return nn.Sequential(
            conv3x3(in_planes, out_planes, stride),
            sn.SwitchNorm2d(out_planes),
...
class C1BilinearDeepSup(nn.Module):
    def __init__(self, num_class=150, fc_dim=2048, use_softmax=False):
        ...
        self.cbr = conv3x3_sn_relu(fc_dim, fc_dim // 4, 1)
        self.cbr_deepsup = conv3x3_sn_relu(fc_dim // 2, fc_dim // 4, 1)
...
class UPerNet(nn.Module):
    def __init__(self, num_class=150, fc_dim=4096,
        ...
        self.ppm_last_conv = conv3x3_sn_relu(fc_dim + len(pool_scales)*512, fpn_dim, 1)
        ...
        for i in range(len(fpn_inplanes) - 1): # skip the top layer
            self.fpn_out.append(nn.Sequential(
                conv3x3_sn_relu(fpn_dim, fpn_dim, 1),
            ))
        self.fpn_out = nn.ModuleList(self.fpn_out)

        self.conv_last = nn.Sequential(
            conv3x3_sn_relu(len(fpn_inplanes) * fpn_dim, fpn_dim, 1),
