...
def train(segmentation_module, iterator, optimizers, history, epoch, args):
    ...
    for i in range(args.epoch_iters):
        ...
        writer.add_scalar('loss_train', loss.data[0], cur_iter)
        writer.add_scalar('mAcc_train', acc.data[0], cur_iter)
        writer.add_scalar('lr_encoder', args.running_lr_encoder, cur_iter)
        writer.add_scalar('lr_decoder', args.running_lr_decoder, cur_iter)

        softmax = nn.Softmax(0)
        
        for name, param in segmentation_module.named_parameters():
           if name.split('.')[1] == 'encoder' and name.split('.')[-1] == 'mean_weight':
               mean_soft = softmax(param.data)  
               writer.add_scalar(name + '_1', mean_soft[0], cur_iter) 
               writer.add_scalar(name + '_2', mean_soft[1], cur_iter)
               writer.add_scalar(name + '_3', mean_soft[2], cur_iter)
           if name.split('.')[1] == 'encoder' and name.split('.')[-1] == 'var_weight':
               var_soft = softmax(param.data)
               writer.add_scalar(name + '_1', var_soft[0], cur_iter) 
               writer.add_scalar(name + '_2', var_soft[1], cur_iter)
               writer.add_scalar(name + '_3', var_soft[2], cur_iter)        
        
        

    writer.add_scalar('ave_loss_train', ave_total_loss.average(), epoch)
    writer.add_scalar('ave_mAcc_train', ave_acc.average(), epoch)
...
def group_weight(module):
    ...
    for m in module.modules():
        if isinstance(m, nn.Linear):
            ...
            if m.mean_weight is not None:
                group_no_decay.append(m.mean_weight)
            if m.var_weight is not None:
                group_no_decay.append(m.var_weight)
